...
namespace impuls
{
	...
	struct component_storage
	{
		...
		i_component* next_valid_component(i32 in_instance_index);
		i_component* previous_valid_component(i32 in_instance_index);

		template <typename component_type>
		static ui32 component_type_index()
		{
			static ui32 index = index_ticker++;
			return index;
		}
		static ui32 index_ticker;
	...
	inline component_type& component_storage::create_component()
	{
		...
		const i32 index = static_cast<int>(m_components.size() / sizeof(component_type));
	...
	inline void component_storage::destroy_component(component_type& in_component_to_destroy)
	{
		...
		for (ui32 i = 0; i < owner->m_components.size(); i++)
		{
