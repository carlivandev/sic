...
void main()
{
	...
	sampler2D ao_r_m_sampler = read_sampler2D(instance_data_texture_sampler, instance_data_begin, instance_data_it);
	...
	vec3 ao_r_m = texture(ao_r_m_sampler, texcoord ).rgb;

	color = calculate_light_for_fragment_pbr(albedo, normal, ao_r_m.z, ao_r_m.y, ao_r_m.x, position_worldspace, eye_direction_cameraspace);
}